...
gulp.task('copybase', done => {
  ...
  gulp.src('./src/css/**/*.png').pipe(gulp.dest('./dist/css')); // flag ,code and author icons
  ...
  gulp.src('./src/download/*.{png,jpg,pdf,gpx}').pipe(gulp.dest('./dist/download')); // downloads
  gulp.src('./src/res/*.json').pipe(gulp.dest('./dist/res')); // dynamic json content
  done();
});

gulp.task('copycustom', done => {
  console.log("Copy custom files");
  gulp.src('./src/js/apod/*.{sqlite,js}').pipe(gulp.dest('./dist/js/apod'));
  gulp.src('./src/js/snake/*.js').pipe(gulp.dest('./dist/js/snake'));
  done();
});
...
gulp.task('build', gulp.series(
  'clean',
  gulp.parallel('copybase', 'copyfonts', 'copycustom'),
  'usemin',
  done => {
